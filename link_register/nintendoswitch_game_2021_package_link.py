...
if __name__ == "__main__":
    ...
    con = mysql_con[0]
    cursor = mysql_con[1]
    switchgame_list : list = DB_operation.select_switchgame_list(cursor, 2021, 1, 1)
    ...
    for count in range(0, len(switchgame_list)):
        ...
        # ダウンロードリンク取得
        if "Rolling Gunner" in switchgame_title or "Ghostrunner" in switchgame_title or "ザ フレンズ オブ リンゴ イシカワ & アレスト オブ ア ストーン ブッダ" in switchgame_title or "ダライアス コズミックリベレーション" in switchgame_title or "エーペックスレジェンズ チャンピオンエディション" in switchgame_title:
            download_link = None
        else:
